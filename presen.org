...
#+begin_quote
*The Physical Symbol System Hypothesis*: A physical symbol system has the necessary and suffcient means for general intelligent action.
#+end_quote
...
+ Physical Symbol ::
  ...
  + Physical patterns in the real world. ( e.g. patterns in the bitmaps of "1"s)
...
(It does not specify what kind of physical patterns they are, but something like the patterns in "1" or ☺)
...
&A;                             /* The content/value disignated by the symbol A */
                                /* We don't know what it is, but it is a physical pattern */
...
;; Using a symbol automatically inserts an entry in a global table.
...
*** Sometimes not removed... but not accessible within programs
...
// --> {plus, a, b, plus, c, d}   i.e. concat produce a new expression.
...
Q: What does A refer to? A: unknown, complex patterns in images, or complex patterns in motore activity e.g. walk
...
Carry out =(+ a b)= : Compute the value of =a + b= numerically
...
** Other essential requirements of sexps : completeness / closure
...
+ There exist processes for creating any expression and for modifying any expression in arbitrary ways.
  + Again, EVAL.
+ *Stableness*: Expressions are stable; once created they will continue to exist until explicitly modified or deleted.
  + Seems obvious?
+ *Unbounded* : The number of expressions that fine system can hold is essentially unbounded.
  + The existence of dynamically allocated memory; Not common in the earliest computers.
...
** The qualitative hypothesis for CS: The Physical Symbol System Hypothesis.
...
+ This is an *empirical* hypothesis.
  ...
  + Just as *Germ hypothesis*, no guarantee that *all* intelligence has a PSS, but *most* or *some* class of systems do.

** The development (history) of PSSH:

Just briefly

+ Formal Logic/Proofs: "symbol game" w/o meaning. Mechanical operations on highly abstracted expressions
+ Turing machines/Information Theory: Defined what is computable. Defined what is information.
...
+ Stored program (After Eniac): In-memory programs
  + program = data. program pointers
  + can designate a process
+ List processing: LISP
  + Dynamic memory allocations + lots of pointer operations (cons cell)
  + Programs were always considered *static* before
  ...
  + All tecnhical requirements were satisfied already, but it wasn't until LISP
    that designation+interpretation is considered as the core concept of
    computation
...
#+begin_row-fluid
#+begin_span6
Sufficiency: Let's build an intelligent system using LISP
...
+ Observe human behavior in intelligent tasks
  + Thinking aloud (thinking while uttering one's thought)
+ Came to agree that human have symbols (short term memory, thunks...)
#+end_span6
#+end_row-fluid
#+end_container-fluid

Other evidences: lack of competing hypothesis.
Others lack the sufficiency criteria; thus lacks the attempt to recreate it
...
Plato / Socrates: Found it difficult how to even /entertain/ problems, let alone solving it.
...
Modern definition of a problem:
...
In practical systems, the efficiency matters. Chess players largely prune nodes. So should other intelligence.

** Conditions for Intelligence

+ Do not resort to brute-force search.
+ Extract information from the test (e.g. goals) and the generator structure (e.g. environment) of the problem
+ Intelligent problems != Large problems.
  + Intelligent problems are exploitable and intelligent system can minimize the search effort.
...
+ Vera, Simon 1993 "Situated Action: A Symbolic Interpretation"
  ...
  + Rebuttal against /Situated Action/ claim

  + Brooks' claim is a leading example of SA
